...
export default class NotificationScreen extends React.Component {
  ...
  onFilePress = (item) => {
    ...
    // Postスクリーンに移動
    navigation.push('Post', { uid: item.post.pid });
  };
    ...
    return (
      <View style={styles.container}>
        <FlatList
              ...
              <View style={styles.row}>
                ...
                >
                  {item.post.type === 'photo' && (
                    <Image uri={item.post.fileUri} style={styles.file} resizeMode={"cover"} />
                  )}
          ...
          ListFooterComponent={() => (
            loading ?
              <View style={styles.loading}><ActivityIndicator size={"small"} /></View> :
              null
          )}
        />
      </View>
